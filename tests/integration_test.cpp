...
protected:
  ...
  std::pair<bool, std::string> compileAndRun(const std::string &source,
    ...
    // Compile
    std::string compile_cmd = compiler_path + " " + source_file.string() +
                              " -o " + output_file.string() + " 2>&1";
    FILE *pipe = popen(compile_cmd.c_str(), "r");
    if (!pipe)
      return {false, "Failed to execute compiler"};
